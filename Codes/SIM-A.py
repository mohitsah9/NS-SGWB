...
F1=np.linspace(20,100,161)


Z1=np.linspace(0.135,10,80)


Mp1=76; t1=0.85; Mp2=80; t2=1; Mp3=84; t3=1.15
  
Mp=Mp2; td=t1

td1=td*hundmyrtosec; k=1;fr=0; a=1.5*M0; b=-1; RA0=30; Mmax=Mp2+60
...
def t(zm):
    y,_=integrate.quad(lambda z:-1/(H*(1+z)*(om*(1+z)**(3)+(1-om))**(1/2)),0, zm) 
    return y # + 4.35*10**(17)
...
def W(zm,m,td,k,a,b,Mp):
    ...
    y1,_ = integrate.quad(lambda z: Ptd1(z,zm,td,k) * Ws(z,m,a,b,Mp) *(1/(H*(1+z)*(om*(1+z)**(3)+(1-om))**(1/2))),zm,30,points=zd,epsabs=tol, epsrel=tol)
...
g=open('4-'+str(Z1[0])+'-ABH'+str(obst)+'Samples='+str(npix)+'RA='+str(RA0)+'Mp='+str(Mp)+'td1='+str(td)+'.txt','w')
...
g=open('2-'+str(Z1[0])+'-ABH'+str(obst)+'Samples='+str(npix)+'RA='+str(RA0)+'Mp='+str(Mp)+'td1='+str(td)+'.txt','w')
